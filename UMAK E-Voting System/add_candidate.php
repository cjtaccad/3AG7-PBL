...
if (isset($_POST['search'])) {
    ...
    if ($uid <= 0) {
        ...
        if ($res && $res->num_rows > 0) {
            ...
            // If student is in 4th year and above or not enrolled, show flash and prevent adding as candidate
            $year = intval($student['year_level']);
            $enrolled = intval($student['is_enrolled']);
            if ($year >= 4 || $enrolled === 0) {
...
(function(){
  ...
  function loadFile(file){
    ...
    reader.onload = function(e){
      ...
      if (croppedInput) croppedInput.value = base64;
    };
...
document.addEventListener('DOMContentLoaded', function(){
  const el = document.querySelector('#flashToastContainer .toast');
  if (!el) return;
  if (window.bootstrap && bootstrap.Toast) {
    const t = new bootstrap.Toast(el, { delay: 3500 });
    t.show();
    el.addEventListener('hidden.bs.toast', () => document.getElementById('flashToastContainer')?.remove());
  } else {
    el.classList.add('show');
    setTimeout(() => document.getElementById('flashToastContainer')?.remove(), 3500);
  }
});
</script>
<?php endif; ?>
